<template>
  <header
    :class="[
      'sticky top-0 z-50 border-b bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60 transition-shadow',
    ...
    >
      ...
      <div class="hidden items-center gap-1 md:flex">
        ...
        <RouterLink
          :to="{ name: 'collab' }"
          class="nav-link"
          :class="{ 'is-active': isActive('/collab') }"
          >組合</RouterLink
        >
        <RouterLink
          :to="{ name: 'counterflow' }"
          class="nav-link"
          :class="{ 'is-active': isActive('/counterflow') }"
          >逆流</RouterLink
        >
        <RouterLink
          :to="{ name: 'roles' }"
          class="nav-link"
          :class="{ 'is-active': isActive('/roles') }"
          >角色</RouterLink
        >
        <RouterLink
          :to="{ name: 'guides' }"
          class="nav-link"
          :class="{ 'is-active': isActive('/guides') }"
          >指南</RouterLink
        >
        <RouterLink
          :to="{ name: 'faq' }"
          ...
          >FAQ</RouterLink
        >
        <RouterLink :to="{ name: 'recommend' }" class="btn-primary ml-1"
          >開始測驗</RouterLink
        >
        ...
        <template v-if="auth.isAuthed">
          <div class="relative">
            ...
            >
              <span
                class="inline-grid h-8 w-8 place-items-center rounded-full bg-slate-200 text-xs font-semibold text-slate-700"
              >
                {{ initials }}
              </span>
            ...
            <Transition name="fade">
              <ul
                v-if="userOpen"
                class="absolute right-0 mt-2 min-w-[180px] overflow-hidden rounded-xl border bg-white/95 p-1 shadow-soft"
              >
                <li>
                ...
                <li>
                  <RouterLink to="/guides/new" class="menu-item"
                    >建立指南</RouterLink
                  >
                </li>
                <li><hr class="my-1 border-slate-200/70" /></li>
                <li>
                  <button class="menu-item text-red-600" @click="logout">
                    登出
                  </button>
                </li>
              </ul>
...
.menu-item {
  @apply flex items-center gap-2 rounded-lg px-3 py-2 text-slate-700 hover:bg-slate-100;
}
.menu-item.is-active {
